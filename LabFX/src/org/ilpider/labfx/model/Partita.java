...
// il layout che contiene i pannelli dei singoli Giocatore
	...
	boolean morto = false;
	listaGiocatori.forEach(g -> g.getListRigaNumero().get(numero).setMorto(false));
	...
	if (giocatoriIsChiuso.count() == listaGiocatori.size()) {
	    ...
	    listaGiocatori.forEach(g -> g.getListRigaNumero().get(numero).setMorto(true));
	}
	return morto;
    }

    public void sommaPunti(int numero) {
	System.out.println("sommo i punti: " + numero);
	if (isNumeroMorto(numero)) {
	    System.out.println("sommo i punti a me");
//	    System.out.println("sss " + listaGiocatori.get(index));
	} else {
	    System.out.println("sommo i punti agli altri");
	    Stream<Giocatore> giocatoriNumeroAperto = listaGiocatori.stream().filter(a -> !a.getListRigaNumero().get(numero).isChiuso());
	    giocatoriNumeroAperto.forEach(g -> g.setPuntiGiocatore(g.getPuntiGiocatore() + numero));
	}
    }
